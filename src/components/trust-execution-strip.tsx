...
export function TrustExecutionStrip({ lang }: TrustExecutionStripProps) {
  ...
  return (
    <section className="border-b border-[#d6dde8] bg-[#f9fbff] py-9">
      <div className="container-shell">
        <div className="rounded-2xl border border-[#dbe3ef] bg-white px-4 py-5 md:px-6 md:py-6">
          ...
          <div className="mt-4 grid gap-2.5 md:grid-cols-3 lg:grid-cols-6">
            {t.trustStrip.map((item) => (
              <article
                key={item}
                className="flex min-h-[54px] items-start rounded-lg border border-[#dbe3ef] bg-[#f8fbff] px-3.5 py-2.5 text-left text-xs font-semibold text-[#153154]"
              >
          ...
          <div className="mt-5 border-t border-[#e6edf6] pt-4">
            ...
            <div className="mt-3 grid gap-3 md:grid-cols-2 xl:grid-cols-5">
              {executionItems.map((item) => (
                <article
                  key={item.title}
                  className="flex min-h-[132px] flex-col rounded-xl border border-[#dbe3ef] bg-[#f8fbff] px-4 py-3 text-left"
                >
                  <div className="mb-2 h-1.5 w-9 rounded-full bg-[#0c9ead]" />
                  <h4 className="text-sm font-semibold text-[#0C1F39]">{item.title}</h4>
                  <p className="mt-1.5 text-xs leading-5 text-[#506987]">{item.body}</p>
                </article>
              ))}
            </div>
          </div>
